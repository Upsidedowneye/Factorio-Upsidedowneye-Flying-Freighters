...
- Spawns a hidden temporary `electric-energy-interface` only while a cargo, unload, resupply, or trash transfer is actively charging; that helper draws at `10MW` by default with assembler/furnace-style priority (`secondary-input`), keeps an intentionally huge prototype-side input-flow limit so runtime code is free to raise or lower individual station rates, and is destroyed as soon as the transfer either completes or is canceled.
- Supports a per-station charge-rate override in the custom station GUI, stored in megawatts and preserved through copy-paste and blueprints; under the current temporary-EEI implementation that override directly changes the one-tick EEI intake window the script exposes, so higher values really do let a station pull more power instead of being clamped back to the default.
...
- Supports a per-station non-negative Supply buffer count in the custom station GUI; Supply stops can reserve that many empty load freighters to pre-position there in advance while waiting for future stock, and if one of those staged freighters boards partial cargo before the source dries up it now hands that cargo off to the next unload leg instead of parking forever at the source. The setting is preserved through ghosts, copy-paste, and blueprints.
...
- The temporary EEI tracks transfer progress by accumulating real energy from the electric network at the station's configured rate, but the cumulative progress now lives on the station record rather than in the EEI buffer itself. Runtime code keeps the EEI's prototype input-flow limit absurdly high, harvests whatever joules the EEI absorbed during the previous tick into that saved progress value, resets the EEI energy back to zero, and then opens only one tick of fresh `electric_buffer_size` headroom. At the default `10MW`, that means the EEI buffer window sits around `166667J` per tick rather than jumping in multi-megajoule chunks every `30` ticks. Partial-power ticks therefore carry forward automatically: if the network only feeds part of the requested wattage during a tick, the transfer simply finishes later instead of snapping between empty/full battery behavior.
...
- Freighter config screens, including the map chooser variant, now also use the green confirm-style Save button, the GUI confirm hotkey (`E`) reuses the same save-and-exit path as the button, and pressing `Shift+Q` copies the currently selected schedule leg into the next journey leg while flipping it between Load and Unload.
...
To keep the current many-to-many network model viable at larger save scales, the runtime now keeps inbound dropoff reservations incrementally, uses short-lived station inventory caches during route scoring, and also caches matched/powered station candidate lists for the rest of the current tick so every idle freighter does not rebuild the same route candidate sets again and again. When two candidate load/unload stops are otherwise equally good, route scoring now also prefers the one with fewer other non-idle freighters already targeting it so networks spread traffic across matching stations instead of dogpiling one stop. The temporary raw station-buffer sizing debug trace has also been removed, so active station transfers no longer spam `factorio-current.log` every tick.
...
- copy/paste and blueprint persistence for both stations and freighters, with vanilla-style shift-click feedback instead of custom chat confirmations, and only when a paste actually changes at least one stored setting.
