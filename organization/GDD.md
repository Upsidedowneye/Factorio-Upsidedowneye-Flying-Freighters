...
- Supports a per-station charge-rate override in the custom station GUI, stored in megawatts and preserved through copy-paste and blueprints; under the current temporary-EEI implementation that override directly changes how much empty EEI buffer the script exposes between power refreshes, so higher values really do let a station pull more power instead of being clamped back to the default.
...
- The temporary EEI tracks transfer progress by accumulating real energy from the electric network at the station's configured rate. Runtime code keeps the EEI's prototype input-flow limit absurdly high, then repeatedly sets `electric_buffer_size` to only the current stored energy plus one update-window of requested charge. Because the network can only fill the empty portion of that buffer before the next refresh, that moving buffer window becomes the real draw ceiling. Partial-power ticks therefore carry forward automatically: if the network only feeds part of the requested wattage during an update span, the transfer simply finishes later instead of snapping between empty/full battery behavior.
